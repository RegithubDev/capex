...
"><c:if test ="${ fn:length(IRMDetails.irmRolesList) eq 0 }" > <input name="level_status" value="Closed"  type="hidden"/>  </c:if>
       ...
       <c:if test ="${fn:length(IRMDetails.irmIncidentsList) eq 0}">
         ...
         <fieldset>
     <div class="table-responsive" <c:if test ="${empty fObj.approver_code }"> </c:if>  
      <c:if test="${fObj.status eq 'Send Back' && sessionScope.USER_ID ne fObj.approver_code || fObj.status eq 'Sent Back' && sessionScope.USER_ID ne fObj.approver_code }" > </c:if>  >
     <script>
     ...
     <c:if test="${sessionScope.USER_ID ne fObj.approver_code   && sessionScope.BASE_ROLE ne 'Admin' && empty fObj.corrective_action }" >  </c:if>  
      ...
      <div class="card">
       ...
       <div class="card-header" style=" margin-left: -2rem;">
        <div class="col-md-12 col-12 mb-1">
          <input name="level_status" value="Pending"  type="hidden"/> 
           <input name="level_code" value="IRL1"  type="hidden"/>
            ...
            <div data-repeater-list="">
             <c:choose>
                ...
                <div class="col-md-2 col-12">
                    <div class="mb-1">
                      <label class="form-label active" for="itemquantity">Remarks<span class="required"> *</span></label>
				                   <c:if test ="${fObj.status eq 'In Progress' }" > <p>
...
<!-- Small Table end -->
         ...
         <c:if test ="${not empty  fObj.approver_type && not fn:contains(fObj.approver_type, 'L1') }"  >
          <div class="row">
            <div class="col-md-6 col-12 mb-1">
              <label class="form-label" for="exampleFormControlTextarea1"><b>Corrective Action</b>
              <c:if test ="${ fn:contains(fObj.approver_type , 'L2') }" ><span class="required"> *</span></c:if></label>
		               <c:if test ="${fObj.status ne 'In Progress' }" > <p > ${fObj.corrective_action }</p> </c:if>
		                  ...
		                  <c:if test ="${fObj.status eq 'In Progress' && fn:contains(fObj.approver_type , 'L2') }" >  <span id="corrective_actionError" class="invalid-feedback" ></span></c:if>
            </div>
              <div class="col-md-6 col-12 mb-1">
              <label class="form-label" for="exampleFormControlTextarea1"><b>Preventive Action</b>
              <c:if test ="${ fn:contains(fObj.approver_type , 'L2') }" ><span class="required"> *</span></c:if></label>
		             <c:if test ="${fObj.status ne 'In Progress' }" > <p >${fObj.preventive_action }</p> </c:if>
		                   ...
		                   <c:if test ="${fObj.status eq 'In Progress' }" > <p>
		                      ...
		                      <c:if test ="${fObj.status ne 'In Progress' || sessionScope.BASE_ROLE eq 'Management'  }" > readonly </c:if>>${fObj.preventive_action }</textarea>
		                       ...
		                       <c:if test ="${fObj.status eq 'In Progress'  && fn:contains(fObj.approver_type , 'L2')}" >  <span id="preventive_actionError" class="invalid-feedback" ></span></c:if>
            </div>
              <div class="col-md-6 col-12 mb-1">
               <label class="form-label" for="exampleFormControlTextarea1"><b>Remarks/Comments</b><span class="required"> </span></label>
				                  <c:if test ="${fObj.status ne 'In Progress' }" > <p > ${fObj.remarks }</p> </c:if>
				                   <c:if test ="${fObj.status eq 'In Progress' }" > <p>
        ...
        <c:if test ="${  fn:contains(fObj.approver_type , 'L2' ) }" >  
         <span class="badge badge-light-info"> Note: To proceed for Level 3, Please Change Status to 'Closed' once Level 2 valuation is Completed. </span> 
         </c:if>
      </div>
     </div>
    <div class="table-responsive">
	                ...
	                <button type="button" class="btn btn-primary me-1" 
	                 ...
	                 <c:if test ="${fn:length(IRMDetails.irmIncidentsList) gt 1  &&  fObj.status eq 'In Progress' }" >
	                  <button type="button" class="btn btn-warning me-1"   onclick="sendBackPopUp();" title="Send Back" data-bs-toggle="modal" data-bs-target="#exampleModalCenter">
	                   ...
	                   <a href="<%=request.getContextPath()%>/irm" class="btn btn-outline-secondary" ><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a>
	                  </c:if>
	                  </c:if>

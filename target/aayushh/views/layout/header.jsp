...
<body>
      ...
      <div class="navbar-container d-flex content">
       <div class="bookmark-wrapper d-flex align-items-center">
        ...
        <ul class="nav navbar-nav align-items-center ms-auto">
            <li class="nav-item" id="performance-info" style="padding-left: 15px;">
			  <span id="internet-speed" style="font-weight:bold; color:orange;">Speed: Checking...</span><br>
			  <span id="browser-warning" style="font-size:small; color:darkred;">Checking system...</span>
			</li>
    ...
    function formatSpeed(speedMbps) {
    	  return (speedMbps >= 1)
    	    ...
    	    : (speedMbps * 1024).toFixed(2) + " Kbps";
    	}


    	function checkInternetSpeed() {
    	  const testUrl = "https://cdn.pixabay.com/photo/2017/05/31/16/39/windows-2360920_1280.png?cache=" + Math.random(); // 100KB image in /images
    	  ...
    	  img.onload = function () {
    	    ...
    	    const speedElem = document.getElementById("internet-speed");

    	    speedElem.textContent = "Speed: " + formatSpeed(speedMbps);
    	    if (speedMbps < 0.5) {
    	      speedElem.style.color = "red";
    	    } else if (speedMbps < 2) {
    	      speedElem.style.color = "orange";
    	    } else {
    	      speedElem.style.color = "green";
    	    }
    	  };

    	  img.onerror = function () {
    	    document.getElementById("internet-speed").textContent = "Speed: No Active Internet Connection";
    	    document.getElementById("internet-speed").style.color = "gray";
    	    document.getElementById("browser-warning").textContent += " ⚠ Cannot load file. Check network or clear cache.";
    	...
    	function checkBrowserAndPerformance() {
    		  const warnElem = document.getElementById("browser-warning");
    		  let msg = "";
    		  ...
    		  if (ua.includes("Chrome")) {
    		    ...
    		    msg += "⚠ Firefox detected – performance may vary. ";
    		  } else if (ua.includes("Edge")) {
    		    msg += "⚠ Edge browser – prefer Chrome. ";
    		  } else {
    		    msg += "⚠ Unsupported browser detected. ";
    		  }

    		  // Memory check (Chrome only, HTTPS required)
    		  if (performance.memory && performance.memory.usedJSHeapSize > 0) {
    		    const used = performance.memory.usedJSHeapSize / 1048576;
    		    const limit = performance.memory.jsHeapSizeLimit / 1048576;
    		    const usagePercent = (used / limit) * 100;

    		    msg += `Memory: ${used.toFixed(1)}MB / ${limit.toFixed(0)}MB (${usagePercent.toFixed(0)}%)`;

    		    if (usagePercent > 90) {
    		      msg += " ⚠ High memory usage!";
    		    }
    		  } else {
    		    msg += "Memory: Info not available (only in Chrome over HTTPS).";
    		  }

    		  warnElem.textContent = msg.trim();
    		}


    	// Initial and repeat every 5 sec
    	checkInternetSpeed();
    	checkBrowserAndPerformance();
    	setInterval(() => {
    	  checkInternetSpeed();
    	  checkBrowserAndPerformance();
    	}, 5000);

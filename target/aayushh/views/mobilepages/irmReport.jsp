    ...
    <div class="app-content content ">
      ...
      <div class="content-wrapper container-xxl p-0">
        <div class="content-header row">
           <div class="content-header-left col-md-8 col-6 mb-2">
          ...
          <div class="content-header-left col-md-2 col-6 mb-2"> 
         	<button class="btn col-md-12" style="margin-top: 17px;   background-color: #14e014 !important;color: white !important;"  onclick="exportIRM();"><i class="fa fa-download" aria-hidden="true"></i> Download</button>
          </div>
        </div>
  ...
  <form action="<%=request.getContextPath()%>/reone/export-irm" name="exportIRMForm" id="exportIRMForm" target="_blank" method="post">	
    ...
    function historyIRM(document_code,approver_type,status,approver_code){
      ...
      function clearFilter(){
			    ...
			    $("#to_date").val("");
			    $("#from_and_to").val("");
		    	window.location.href= "<%=request.getContextPath()%>/reone/irm-report";
	    }
      ...
      function getSBUFilterList() {
  	    ...
  	    var project_code = $("#select2-Project_filter-container").val();
	        ...
	        if ($.trim(sbu_code) == "") {
	            ...
	            $.ajax({
	                url: "<%=request.getContextPath()%>/reone/ajax/getSBUFilterListFromIRMReport",
      ...
      function getProjectFilterList() {
    	  $(".page-loader").show();
	        ...
	        if ($.trim(project_code) == "") {
	            ...
	            $.ajax({
	                url: "<%=request.getContextPath()%>/reone/ajax/getProjectFilterListFromIRMReport",
	                data: myParams, cache: false,async: true,
	                success: function (data) {
	                    if (data.length > 0) {
	                        $.each(data, function (i, val) {
      ...
      function getIncidentFilterList() {
    	  $(".page-loader").show();
	        ...
	        if ($.trim(incident_type) == "") {
	            ...
	            $.ajax({
	                url: "<%=request.getContextPath()%>/reone/ajax/getIncidentFilterListFromIRMReport",
	                data: myParams, cache: false,async: true,
	                success: function (data) {
	                    if (data.length > 0) {
	                        $.each(data, function (i, val) {
      ...
      function getStatusFilterList() {
    	  ...
    	  var project_code = $("#select2-Project_filter-container").val();
	        ...
	        if ($.trim(status) == "") {
	            ...
	            $.ajax({
	                url: "<%=request.getContextPath()%>/reone/ajax/getStatusFilterListFromIRMReport",
	                data: myParams, cache: false,async: true,
	                success: function (data) {
	                    if (data.length > 0) {
	                        $.each(data, function (i, val) {
	    ...
	    function getIRMList(){
			...
			$.ajax({url : "<%=request.getContextPath()%>/reone/ajax/getIRMListReport",type:"POST",data:myParams,success : function(data){    				
					if((data != null && data != '' && data.length > 0 ) 

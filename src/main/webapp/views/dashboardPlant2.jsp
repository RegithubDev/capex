							...
							<h4>Project Manager / Execution</h4>
							 <div class="card-header">
							        ...
							        <c:if test="${not empty editList.project_manager_date}">
							           ...
							           <input type="hidden" name="current_pending_at" id="head_of_plant_name" />
							       
							    </div>
							<div class="signature-fields">
							<c:if test="${not empty fn:trim(editList.project_manager_date)}">
							<select name="project_manager_name" id="projectManager" 
							        data-original="${editList.project_manager_name}" required>
							...
							<div class="signature-card">
							    <div class="card-header">
							        ...
							        <c:if test="${not empty editList.head_of_plant_name}">
							            ...
							            <input type="hidden" name="current_pending_at" id="finance_name" />
							    </div>
							    ...
							    <span style="color:#4e28a5 !important; font-size:14px; margin-left:10px;">
							        Signed on ${editList.head_of_plant_date}
							    </span>
							 <c:if test="${editList.finance_status ne 'Rejected' && (sessionScope.BASE_ROLE eq 'Admin' || sessionScope.USER_ID eq editList.current_pending_at)}">
							    ...
							    <div style="margin-top:8px; font-size:13px; color:#555;">
							        <b>Note:</b> The Project Manager may be updated if required. Authorized personnel can modify the Project Manager and resubmit the proposal for approval.
							    </div>
						    </c:if>
...
<section class="capex-card finance-section approval-stage">
        ...
        <div class="finance-card">
            ...
            <div class="finance-field">
                ...
                <select name="finance_name" required>
                    <option value="">Select Name</option>
                    <option value="${editList.getPlantHeade_name}" selected>${editList.finance_name}</option>
                </select>
            </div>
            <div class="finance-field">
				...
				<c:if
					test="${((editList.finance_status eq 'Approved') or (editList.department eq 'GN' ) && not empty editList.project_manager_date)}">
					<section class="capex-card approval-stage finance-controller-section">
...
<div class="signature-card ">
            ...
            <c:when test="${editList.department ne 'OPS'}">
                ...
                <c:if test="${not empty fn:trim(editList.head_projects_name)}">
                    <div class="signature-card">
                        ...
                        <div class="signature-fields business-head-section approval-stage">
        ...
        <c:if test="${not empty fn:trim(editList.cfo_name)}">
            <div class="signature-card ceo-section approval-stage">
                <div class="card-header">
                    <h4>CEO & MD</h4>
		...
		<div class="action-bar">
		    <c:if test="${ (sessionScope.BASE_ROLE eq 'Admin' || sessionScope.USER_ID eq editList.current_pending_at)}">
		        <button type="submit" class="submit-btn">
		            <i class="material-icons">${not empty editList ? 'save' : 'send'}</i>
		            ${not empty editList ? 'Update' : 'Submit Proposal'}
		        </button>
		    </c:if>
...
$(document).ready(function() {
    ...
    $("#financeStatus").on("change", function() {
        ...
        var textarea = $("#financeComments");
			...
			if (status === "Rejected") {
			    ...
			    // remove name from all current_pending_at fields
			   ...
			   // $("#finance_name").attr("name", "current_pending_at");
			    ...
			    // remove name from existing fields
			    $("[name='current_pending_at']").removeAttr("name");

			    // assign next approver
			    $("#regional_director_name").attr("name", "current_pending_at");

			    // remove finance hidden input completely
			    $("#finance_name").remove();
 ...
 // Get the form element
    ...
    pendingElements.forEach((el, index) => {
        ...
        console.log('Index '+index+': current_pending_at = '+el.id+', lastField ('+lastField.name+') = '+lastField.id);
    });
});
document.addEventListener('DOMContentLoaded', function () {

    const department = $("#department").val() || "";
    const formId = $("#capexId").val() || "";
    ...
    console.log("Final Approval Section:", finalApproverClass);
    ...
    if (department === "GN") {
        ...
        gnRemoveIds.forEach(function(id){
    ...
    /* SUBMIT BUTTON FINAL CLASS */

    const submitBtn = $(".submit-btn");

    submitBtn.removeClass(
        "final-approval finance-section finance-controller-section business-head-section ceo-section"
    );
 ...
 submitBtn.off("click").on("click", function (e) {
    ...
    $.ajax({
        url: "/capex/form/ajax/getApprovalStatus",
        type: "POST",
        data: {
            capexId: formId,
            lakhs: lakhs,
            department: '${editList.department}'
        },
        ...
        success: function (response) {
            ...
            /* EXISTING APPROVAL VALUES */

            let approvals = {
                head_of_plant_name: '${editList.head_of_plant_name}',
                finance_name: '${editList.finance_name}',
                regional_director_name: '${editList.regional_director_name}',
                finance_controller_name: '${editList.finance_controller_name}',
                head_projects_name: '${editList.head_projects_name}',
                business_head_name: '${editList.business_head_name}',
                cfo_name: '${editList.cfo_name}',
                ceo_name: '${editList.ceo_name}'
            };
            ...
            rules.forEach(function (rule) {
                ...
                if (rule.previous_required_field && rule.previous_required_field !== "NULL") {
                    approvalChain.push(rule.previous_required_field);
                }

                if (rule.final_approver && rule.final_approver !== "NULL") {
                    approvalChain.push(rule.final_approver);
                }
            ...
            approvalChain = [...new Set(approvalChain)];
            ...
            for (let i = 0; i < approvalChain.length; i++) {
                ...
                let approver = approvalChain[i];
                let value = approvals[approver];

                if (!value || value.trim() === "") {

                    if (i + 1 < approvalChain.length) {
                        nextApproverId = approvalChain[i + 1];
                    }
            ...
            console.log("Next Approver:", nextApproverId);
            ...
            if (nextApproverId) {
                ...
                $("#" + nextApproverId).attr("name", "current_pending_at");
            }
        ...
        error: function (err) {
            console.log("Approval rule fetch failed", err);
        }
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if(!pmSelect || !submitBtn) return;

    const originalPM = pmSelect.dataset.original || "";

    pmSelect.addEventListener("change", function(){

        const newPM = this.value;

        if(originalPM && newPM !== originalPM){

            submitBtn.innerHTML =
            '<i class="material-icons">edit</i> Update Project Manager';
